...
function createStore(pool) {
    ...
    const methods = {
        ...
        async recordClick(codigo) {
            return transaction(async db => {
                const { rows } = await db.query('SELECT status FROM public.afiliados WHERE codigo = $1 FOR SHARE', [codigo]);
                if (rows[0]?.status !== 'ativo') return false;
                await db.query('INSERT INTO public.cliques_afiliados (id, afiliado_codigo) VALUES ($1,$2)', [id('clk'), codigo]);
                return true;
            });
        },
